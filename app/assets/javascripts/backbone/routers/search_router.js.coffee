class Site.Routers.SearchesRouter extends Backbone.Router
	...
	pesquisa: (query) ->
		...
		window.query = query
		window.page = 1
		window.busca.url = "http://www.buscacontagem.com.br/api/anuncios/busca/" + query + "/"
		window.busca.load()
	...
	paged: (query,page) ->
		...
		window.busca.url = "http://www.buscacontagem.com.br/api/anuncios/busca/" + query + "/page=" + page + "/"
		window.busca.load()